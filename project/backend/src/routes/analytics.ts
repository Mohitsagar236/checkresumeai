...
router.get('/trends', asyncHandler(async (req: Request, res: Response) => {
  ...
  // Format trends data based on requested metric
  const formattedTrends = trends?.map(trend => ({
    timestamp: trend.timestamp,
    value: trend[metric as keyof typeof trend] || 0,
...
router.get('/benchmark', asyncHandler(async (req: Request, res: Response) => {
  const userId = req.user!.id;
  ...
  // Industry benchmarks (in a real app, this would come from a database)
  ...
  const benchmark = industryBenchmarks[industry as string] || industryBenchmarks.general;
...
router.get('/export', asyncHandler(async (req: Request, res: Response) => {
  ...
  const [analyticsResult, trendsResult, analysesResult] = await Promise.all([
    ...
    (async () => {
      ...
      if (startDate) trendsQuery = trendsQuery.gte('timestamp', startDate);
      if (endDate) trendsQuery = trendsQuery.lte('timestamp', endDate);
      
      return trendsQuery.order('timestamp');
    })(),
      // Get analyses data with filters
    (async () => {
      ...
      if (startDate) analysesQuery = analysesQuery.gte('created_at', startDate);
      if (endDate) analysesQuery = analysesQuery.lte('created_at', endDate);
...
router.get('/admin/summary', asyncHandler(async (req: Request, res: Response) => {
  ...
  res.json({
    ...
    data: {
      ...
      systemHealth: 'good' // This would be computed based on various metrics
    },
  });
}));
...
function convertToCSV(data: any): string {
  ...
  const rows = trends.map((trend: any) => Object.values(trend).join(','));
