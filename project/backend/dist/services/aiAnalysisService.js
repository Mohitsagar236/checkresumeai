...
`;
export const analyzeResumeWithAI = async (resumeText, jobRole, analysisType = 'comprehensive') => {
    ...
    try {
        ...
        if (config.ai.groq.apiKey) {
            ...
            const completion = await groq.chat.completions.create({
                ...
                max_tokens: Math.min(config.ai.groq.maxTokens, 8000),
                temperature: 0.3,
            });
            const response = completion.choices[0]?.message?.content;
            if (!response) {
...
const generateFallbackAnalysis = (resumeText, jobRole) => {
    ...
    return {
        ...
        estimatedReading: {
            ...
            difficulty: wordCount > 400 ? 'medium' : 'easy'
        }
    };
};
