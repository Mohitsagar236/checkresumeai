...
export const usePayment = (planType: keyof typeof RAZORPAY_PLANS, onSuccess?: () => void) => {
      ...
      const options = {
        ...
        config: {
          ...
          verifyRazorpayPayment(
            ...
            if (verified) {              
              // Update subscription to premium when payment is verified              
              ...
              console.log('Payment verified - updating subscription with data:', newSubscriptionData);
              updateSubscription(newSubscriptionData);
              setPaymentStep('success');
              showToast('Payment successful! Premium features unlocked!', 'success');
              
              // Call onSuccess after a short delay to ensure state updates propagate
              setTimeout(() => {
                onSuccess?.();
              }, 100);
            } else {
  ...
  }, [user, planType, plan, promoDiscount, onSuccess, showToast, setProcessingPayment]);  const resetPayment = useCallback(() => {
    setPaymentStep('details');
    setPromoCode('');
    setPromoDiscount(0);
    setPromoError('');
