...
// Mock course database - in a real app, this would be from a database or external API
const COURSE_DATABASE: Omit<CourseRecommendation, 'relevanceScore' | 'reasons'>[] = [
  // Technical Skills
  {
    id: 'js-fundamentals',
...
const JOB_ROLE_SKILLS: { [key: string]: string[] } = {
...
export const generateCourseRecommendations = async (
  ...
  if (skillsGap && skillsGap.length > 0) {
    ...
    targetSkills = JOB_ROLE_SKILLS[jobRole];  } else {
    targetSkills = JOB_ROLE_SKILLS.general || [];
  }

  // Filter courses based on category if specified
  let filteredCourses = category 
    ? COURSE_DATABASE.filter(course => course.category === category)
    : COURSE_DATABASE;

  // Calculate relevance scores
  const recommendations: CourseRecommendation[] = filteredCourses.map(course => {
...
const calculateRelevanceScore = (
  course: Omit<CourseRecommendation, 'relevanceScore' | 'reasons'>,
  targetSkills: string[],
  analysisData?: ResumeAnalysisResult
): number => {
  let score = 0;
  ...
  // Course rating (20% of score)
  score += (course.rating / 5) * 20;

  // Review count factor (10% of score)
  const reviewScore = Math.min(course.reviewCount / 10000, 1) * 10;
  score += reviewScore;

  // Free courses get a slight boost (5% of score)
  ...
  if (analysisData) {
    ...
    if (userLevel < 60 && course.difficulty === 'beginner') difficultyScore = 15;
    else if (userLevel >= 60 && userLevel < 80 && course.difficulty === 'intermediate') difficultyScore = 15;
    else if (userLevel >= 80 && course.difficulty === 'advanced') difficultyScore = 15;
    else difficultyScore = 5; // Partial match
    
    score += difficultyScore;
  } else {
    score += 10; // Default if no analysis data
  }

  // Provider reputation (10% of score)
